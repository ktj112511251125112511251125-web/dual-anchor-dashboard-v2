...
# 예시:
#   python QQQ_QLD_TQQQ_telegram_mobile_cmd_v14_live_judgement.py --cash 10000 --qld-shares 5 --qld-avg-price 90


# ======================================================
# QQQ / QLD / TQQQ 최종 전략 - Windows CMD / 터미널 실행용
# v14-live: 종가 판단 + 실시간 현재가 판단
...
STRATEGY_NAME = "DD쿨차등_C_17_19_21_24_C20"
...
OUTPUT_DIR = os.path.abspath("qqq_qld_tqqq_v14_live_results")

def parse_args():
    parser = argparse.ArgumentParser(description="QQQ/QLD/TQQQ v14 live judgement - CMD executable script")
    parser.add_argument("--cash", type=float, default=CASH_MANUAL, help="현금")
    parser.add_argument("--qqq-shares", type=float, default=HOLDINGS_MANUAL["QQQ"], help="QQQ 보유수량")
    parser.add_argument("--qld-shares", type=float, default=HOLDINGS_MANUAL["QLD"], help="QLD 보유수량")
    parser.add_argument("--tqqq-shares", type=float, default=HOLDINGS_MANUAL["TQQQ"], help="TQQQ 보유수량")
    parser.add_argument("--qqq-avg-price", type=float, default=AVG_PRICE_MANUAL["QQQ"], help="QQQ 평균단가")
    parser.add_argument("--qld-avg-price", type=float, default=AVG_PRICE_MANUAL["QLD"], help="QLD 평균단가")
    parser.add_argument("--tqqq-avg-price", type=float, default=AVG_PRICE_MANUAL["TQQQ"], help="TQQQ 평균단가")
    parser.add_argument("--sold-today", action="store_true", help="오늘 이미 매도한 것으로 처리")
    parser.add_argument("--cooldown-days", type=int, default=COOLDOWN_DAYS_LEFT, help="남은 쿨다운 거래일")
    parser.add_argument("--output-dir", default=OUTPUT_DIR, help="결과 파일 저장 폴더")
    parser.add_argument("--send-telegram", action="store_true", help="텔레그램 발송")
...
def draw_yellow_footer(draw, W, y, line1="상승장=TQQQ · 조정장=QLD", line2="하락장=QQQ/QLD · 목표수익=전량익절"):
...
print("데이터 다운로드 중...")
qqq = download_ohlcv("QQQ")
qld = download_ohlcv("QLD")
tqqq = download_ohlcv("TQQQ")
...
def decide_buy_signal_context(row, sold_today_flag, auto_sell_signal_flag, position_return_value):
    ...
    if sold_today_flag:
        if auto_sell_signal_flag:
            reason = f"익절 조건 달성. {holding_combo} 현재 수익률 {position_return_value*100:.2f}% / 목표 {target_return*100:.2f}%. 오늘 전량매도, 오늘 재매수 없음."
        else:
            reason = "오늘 이미 매도 처리됨. 당일 재매수 없음."
    ...
    if above_ma200 and ma50_up:
        ...
        if COOLDOWN_DAYS_LEFT > 0:
            reason = f"강한 상승장 / 익절 후 쿨다운 {COOLDOWN_DAYS_LEFT}일 남음 / TQQQ {div}분할"
        return {"action":"BUY", "market_type":"강한 상승장", "reason":reason, "divisions":div, "mix":{"TQQQ":1.0}, "dd_band":dd_band}
...
def build_order_df(sig, price_map, equity_value):
    rows = []
    if sig["action"] == "BUY" and sig["divisions"] is not None and CASH > 0:
        ...
        for ticker, weight in sig["mix"].items():
            ...
            rows.append({"Ticker": ticker, "Weight": weight, "Buy_Amount": round(amount,2), "Price": round(price,2), "Estimated_Shares": round(amount/price,6) if price > 0 else 0.0})
...
def make_live_final_status(daily_sig, live_sig):
    ...
    if live_sig["action"] == "SELL_AND_NO_BUY":
        return {
            "status": "LIVE_SELL",
            "title": "실시간 매도",
            "final": "실시간 현재가 기준으로 익절/매도 조건이 충족되었습니다. 전량매도 후 오늘 재매수 금지 기준을 확인하세요.",
...
def create_today_action_card(path=os.path.join(OUTPUT_DIR, "today_action_card.png")):
    ...
    row_y = header_y + 82
    if signal["action"] == "SELL_AND_NO_BUY":
        rounded(d,(90,row_y,990,row_y+125),radius=28,fill=CARD["sell_soft"])
        d.text((125,row_y+45),"매도",font=get_font(30),fill=CARD["sell"],anchor="lm"); d.text((260,row_y+45),"전체",font=get_font(30),fill=CARD["text"],anchor="lm")
        d.text((410,row_y+45),"-",font=get_font(26),fill=CARD["text"],anchor="lm"); d.text((610,row_y+45),"전량",font=get_font(26),fill=CARD["sell"],anchor="lm"); d.text((905,row_y+45),"ALL",font=get_font(26),fill=CARD["sell"],anchor="mm")
        d.text((125,row_y+92),"매도 후 오늘은 재매수하지 않습니다.",font=get_font(23),fill=CARD["subtext"])
    elif signal["action"] == "BUY" and not buy_df.empty:
        for _, row in buy_df.iterrows():
            rounded(d,(90,row_y,990,row_y+125),radius=28,fill=CARD["buy_soft"])
            d.text((125,row_y+43),"매수",font=get_font(30),fill=CARD["buy"],anchor="lm"); d.text((260,row_y+43),row["Ticker"],font=get_font(31),fill=CARD["text"],anchor="lm")
            d.text((410,row_y+43),money(row["Price"]),font=get_font(24),fill=CARD["text"],anchor="lm"); d.text((610,row_y+43),money(row["Buy_Amount"]),font=get_font(24),fill=CARD["buy_dark"],anchor="lm"); d.text((905,row_y+43),str(row["Estimated_Shares"]),font=get_font(21),fill=CARD["text"],anchor="mm")
            d.text((125,row_y+92),f"{signal['divisions']}분할 기준 · 총자산 기준 분할매수",font=get_font(22),fill=CARD["subtext"])
            row_y += 140
    else:
    ...
    if signal["action"]=="SELL_AND_NO_BUY": cx=chip(d,cx,cy,"재매수 없음",CARD["sell_soft"],CARD["sell"],font_size=22)
    draw_text_box(d,95,1205,signal["reason"],get_font(28),CARD["text"],850,line_gap=12,max_lines=3)
    ...
    kv(d,95,1570,"쿨다운",f"{COOLDOWN_DAYS_LEFT}일",value_color=CARD["wait"],size=28)
...
def create_live_check_card(path=os.path.join(OUTPUT_DIR, "live_execution_check_card.png")):
    ...
    draw_text_box(d,95,1285,live_final["final"],get_font(27),CARD["text"],850,line_gap=12,max_lines=4)
    ...
    y=1585
    if live_signal["action"] == "SELL_AND_NO_BUY":
        rounded(d,(95,y,965,y+92),radius=26,fill=CARD["sell_soft"])
        d.text((130,y+46),"전량매도",font=get_font(30),fill=CARD["sell"],anchor="lm")
        d.text((930,y+46),"ALL",font=get_font(30),fill=CARD["sell"],anchor="rm")
    elif not live_buy_df.empty:
        for _, row in live_buy_df.iterrows():
            rounded(d,(95,y,965,y+82),radius=26,fill=CARD["buy_soft"])
            d.text((130,y+26),row["Ticker"],font=get_font(27),fill=CARD["text"],anchor="lm")
            d.text((330,y+26),money(row["Price"]),font=get_font(23),fill=CARD["blue"],anchor="lm")
            d.text((540,y+26),money(row["Buy_Amount"]),font=get_font(23),fill=CARD["buy_dark"],anchor="lm")
            d.text((930,y+26),str(row["Estimated_Shares"]),font=get_font(20),fill=CARD["text"],anchor="rm")
            d.text((130,y+60),"현재가 기준 예상 수량",font=get_font(18),fill=CARD["subtext"],anchor="lm")
            y += 92
    else:
...
for p in [today_card_path, live_card_path, analysis_card_path, strategy_card_path, qqq_price_graph_path, drawdown_graph_path, buy_order_graph_path]:
    print("이미지 생성:", p)
...
쿨다운: {COOLDOWN_DAYS_LEFT}일

오늘 주문:
"""

if signal["action"] == "SELL_AND_NO_BUY":
    telegram_text += "\n전량매도하세요.\n오늘은 재매수하지 않습니다.\n다음 거래일부터 쿨다운 7거래일 적용하세요.\n"
elif signal["action"] == "BUY" and not buy_df.empty:
    for _, row in buy_df.iterrows():
